...
double bkg_Na(double *x, double *p) {
  if (reject2 && x[0]>=51e3 && x[0]<=57.5e3) {
    TF1::RejectPoint();
    return 0;
  }
  return p[0] + p[1] * x[0] + p[2] * x[0] * x[0];
...
TFitResultPtr Cobalt2() {
  ...
  double LowLim = 53e3, UpLim = 60e3;
  double bkgMin = 60e3, bkgMax = 60e3;
  int nbins = 100;
  ...
  TF1 *fitFunc = new TF1("fit1274.5.3Func", f_Co2, LowLim, UpLim, 5);
...
TFitResultPtr Sodium() {
  ...
  std::string treepath = "data/22Na.root",
              figpath = "figures/calibrationTrap/22Na.pdf",
              elementname = "{}^{22}Na";

  double LowLim = 45e3, UpLim = 62e3;
  double peakMin = 45e3, peakMax = 62e3;
  int nbins = 100;
  ...
  fitFunc->SetParameters(.4e3, 54e3, .8e3,
                         bkg->GetParameter(0), bkg->GetParameter(1),
                         bkg->GetParameter(2));
